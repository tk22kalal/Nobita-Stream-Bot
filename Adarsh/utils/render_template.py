...
async def render_generate_page(id, secure_hash):
    ...
    stream_link = urllib.parse.urljoin(Var.URL, f'{secure_hash}{str(id)}')
    download_link = urllib.parse.urljoin(Var.URL, f'dl/{secure_hash}{str(id)}')

    async with aiofiles.open('Adarsh/template/generate.html') as r:
        html = await r.read()
        html = html.replace('{{ stream_link }}', stream_link)
